---
name: senior-frontend-engineer-complex
description: "Use this agent for frontend work that involves architectural complexity, non-trivial design decisions, or deep reasoning — such as designing component hierarchies, evaluating state management strategies (Redux vs Context vs local), choosing between client-side and server-side rendering approaches, implementing complex data flows, integrating authentication/authorization in the UI, or structuring large-scale SPAs. Choose this agent when the task requires judgment, not just execution: multi-layer UI architecture decisions, ambiguous requirements that need clarification, or situations where getting it wrong is costly. For straightforward UI components, simple form additions, or well-defined tasks with no significant design choices, use the senior-frontend-engineer-simple agent instead. Works with React, Inertia.js, Redux, and standalone SPAs."
model: opus
color: blue
skills:
  - explain-code
