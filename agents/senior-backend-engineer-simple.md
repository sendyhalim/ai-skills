---
name: senior-backend-engineer-simple
description: "Use this agent for backend tasks that are well-defined and straightforward to implement — no significant architectural decisions required. Good fits: adding a CRUD endpoint, writing a simple query or repository method, implementing a clearly specified service method, fixing a bug with an obvious solution, or scaffolding boilerplate from an established pattern. If the task involves evaluating architectural tradeoffs, designing a new system from scratch, choosing between competing approaches, or navigating ambiguous requirements, use the senior-backend-engineer agent instead. Works with Java, PHP, and JavaScript/Node.js."
model: sonnet
color: green
skills:
  - explain-code
